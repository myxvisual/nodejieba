...
async function test() {
    const Module = await emModule();
    
    // Module['FS_createDataFile']("/tmp", "test.txt", "test-content", true, true)
    // Module.testReadFile();
    ...
    const now = Date.now()
    const loadRes = Module.load(DEFAULT_DICT, DEFAULT_HMM_DICT, DEFAULT_USER_DICT, DEFAULT_IDF_DICT, DEFAULT_STOP_WORD_DICT)
    console.log(loadRes)
    console.log(`${Date.now() - now}ms`)
    // ~3616 ms --> ~2789ms

    const cutVec = Module.cut("南京市长江大桥", false);
    const cutArr = vectorToArray(cutVec)
    console.log(cutArr)
    const tagMap = Module.tag("红掌拨清波")
